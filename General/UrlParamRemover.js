...
describe('removeFooFromUrl', () => {
    ...
    it('should remove `foo` if it is present as any query parameter', () => {
        expect(
            removeFooFromUrl('https://www.segment.com?a=b&foo=bar&c=d')
        ).toEqual('https://www.segment.com?a=b&c=d');
    });

    it('should remove multiple occurrences of the query param foo', () => {
        expect(
            removeFooFromUrl('https://www.segment.com?foo=bar&foo=baz')
        ).toEqual('https://www.segment.com');
    });
